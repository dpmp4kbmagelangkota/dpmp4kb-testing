...
function openArticle(id){const a=articles.find(x=>String(x.id)===String(id));if(!a)return;detail.innerHTML=`${a.image?`<img class="detail-cover" src="${esc(a.image)}" alt="${esc(a.title)}">`:""}<div class="meta">${esc(a.category)} • ${formatDate(a.date)} • ${esc(a.author)}</div><h1>${esc(a.title)}</h1><p class="detail-excerpt">${esc(a.excerpt)}</p><div class="article-content">${a.content||""}</div>`;dialog.showModal()}
